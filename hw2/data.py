...
def sample_data(data_name='circle', nTrain=200, nTest=200):
    """
...
class DataLoader:
    def __init__(self, data: np.ndarray, labels: np.ndarray, batch_size: int, shuffle: bool=False, is_finite: bool=True):
        self.data = data
        self.labels = labels
    ...
    def __next__(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        start = self._idx
        end = min(self._idx + self.batch_size, self.n_samples)
