...
class LSGANGANTrainer(BaseGANTrainer):
    """Least-squares GAN using the baseline DCGAN architecture"""
    ...
    def train_step(self, G, D, real_images, g_optimizer, d_optimizer):
        ...
        D_real_loss = 0.5 * torch.nn.functional.mse_loss(real_scores, torch.ones_like(real_scores))

        noise = self.sample_noise(real_images.size(0))
        fake_images = G(noise)
        fake_scores = D(fake_images.detach())
        D_fake_loss = 0.5 * torch.nn.functional.mse_loss(fake_scores, torch.zeros_like(fake_scores))
        ...
        G_loss = 0.5 * torch.nn.functional.mse_loss(fake_scores, torch.ones_like(fake_scores))
